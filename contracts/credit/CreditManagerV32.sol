...
contract CreditManagerV32 is ICreditManagerV3, SanityCheckTrait, ReentrancyGuardTrait {
        ...
        returns (uint256 remainingFunds, uint256 loss)
    {
        ...
        if (amountToPool != 0) {
            IERC20(underlying).safeTransferFrom({from: creditAccount, to: pool, amount: amountToPool});
        ...
        returns (uint256 newDebt, uint256, uint256)
    {
        ...
        if (action == ManageDebtAction.INCREASE_DEBT) {
            ...
            IERC20(underlying).safeTransferFrom({from: creditAccount, to: pool, amount: amount});
