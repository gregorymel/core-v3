...
contract CreditFacadeV32UnitTest is TestHelper, BalanceHelper, ICreditFacadeV3Events {
    ...
    // function _multicallInt(address creditAccount, MultiCall[] memory calls, uint256 enabledTokensMask) internal {
    //     creditManagerMock.setEnabledTokensMask(enabledTokensMask);

    //     vm.startPrank(creditAccount);
    //     creditFacade.checkBeforeExecution();
    //     {
    ...
    //         }
    //     }
    //     creditFacade.checkAfterExecution();
    //     vm.stopPrank();
    // }
    ...
    function test_U_FA_24_multicall_setFullCheckParams_works_properly() public notExpirableCase {
        ...
        vm.expectCall(
            ...
            abi.encodeCall(
                ...
                (creditAccount, UNDERLYING_TOKEN_MASK, collateralHints, minHealthFactor, true)
            )
        );
        creditFacade.multicallInt({
            creditAccount: creditAccount,
    ...
    function test_U_FA_34_multicall_updateQuota_works_properly() public notExpirableCase {
        ...
        vm.expectCall(
            ...
            abi.encodeCall(
                creditManagerMock.fullCollateralCheck,
                (creditAccount, maskToEnable | UNDERLYING_TOKEN_MASK, new uint256[](0), PERCENTAGE_FACTOR, true)
            )
        );
        creditFacade.multicallInt({
            creditAccount: creditAccount,
